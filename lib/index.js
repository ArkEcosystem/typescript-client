...
module.exports = class ApiClient {
  ...
  static async findPeers (network, version = 1, peersOverride) {
    ...
    for (const peer of networkPeers) {
      ...
      try {
        ...
        const { body } = await client.resource('peers').all()

        let responsePeers = []
        if (version === 1 && body.success && body.peers) {
          responsePeers = body.peers
        } else if (version === 2 && body.data.length) {
          responsePeers = body.data
        }
  ...
  static async fetchPeerConfig (host) {
    ...
    for (const attempt of attempts) {
      ...
      try {
        const { body } = await httpClient.sendRequest('get', attempt.endpoint)
        if (body) {
          return body.data
        }
      } catch (error) {
